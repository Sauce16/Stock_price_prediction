...
def getInputData(fileName):
    ...
    inputData = np.array([price[2:-1], twoMonthMA, threeMonthMA, size, classification], dtype=object).T
...
if __name__ == '__main__':
    ...
    inflation = list(map(float, [item.rstrip('%') for sublist in inflation for item in sublist]))
    ...
    clusterData = np.array([lst + [0]*(max_len - len(lst)) for lst in [unemployment, inflation, djia, sp]]).T
